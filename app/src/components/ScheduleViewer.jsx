...
import { Calendar, FileText, Files, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function ScheduleViewer({
  ...
  const isRtl = lang === 'ar';
  const displayInstructor = (sec) => {
    const name = lang === 'ar' && sec.instructorAr ? sec.instructorAr : sec.instructor;
    return normalizeInstructor(name);
  };

  // Add Course Details table — spruced, English for print, includes college
  const addCourseDetailsTable = (doc, scheduleData, startY) => {
    ...
    const tableBody = scheduleData.map(section => {
      const dept = section.department || section.college || '';
      const shortDept = dept ? dept.replace('Department of ', '').slice(0, 18) : '';
      return [
        section.code,
    ...
    pdfModules.autoTable(doc, {
      startY: y,
      head: [['Code', 'Course Name', 'Dept', 'Sec', 'Instructor', 'Room']],
      body: tableBody,
      margin: { left: PDF_STYLES.MARGIN, right: PDF_STYLES.MARGIN },
      styles: {
  ...
  // Add exam schedule table — English for print fidelity
  const addExamScheduleTable = (doc, scheduleData, startY) => {
    ...
    const tableBody = scheduleData.map(section => {
      ...
      return [section.code, section.name || section.nameEn || '', date, time];
    });
  ...
  // Export single schedule — spruced, English for print fidelity
              ...
              <React.Fragment key={slotKey}>
                ...
                {[0, 1, 2, 3, 4].map(dayIdx => {
                  ...
                  return (
                    <div
                      key={dayIdx}
                      className={`day-cell ${classes.length > 0 ? 'has-class' : ''}`}
                    >
                      {classes.map((cls, clsIdx) => {
                        ...
                        return (
                          <div
                            ...
                            title={`${cls.code} - ${displayInstructor(cls)} | ${roomLabel}`}
                          >
                            ...
                            <div className="block-instructor">{displayInstructor(cls)}</div>
                          </div>
                        );
                      })}
                    </div>
