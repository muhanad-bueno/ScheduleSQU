...
export default memo(function FilterPanel({ blockedSlots, onToggleSlot, onToggleDay, onReset, t }) {
  ...
  return (
    <div className="filter-panel">
      <button className="filter-toggle" onClick={() => setExpanded(!expanded)} aria-expanded={expanded}>
        <span className="filter-toggle-text">
          <Clock size={18} />
          <span>{t.filterTitle}</span>
        </span>
        <span className={`filter-chevron ${expanded ? 'open' : ''}`}>
          <ChevronDown size={16} />
        </span>
      </button>

      <div className={`filter-collapsible ${expanded ? 'open' : ''}`} aria-hidden={!expanded}>
